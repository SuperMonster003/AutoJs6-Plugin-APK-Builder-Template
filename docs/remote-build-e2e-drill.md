...
| `customKeyStoreV2IconAndAbiSelectionComplete` | Single JavaScript file + custom BKS FD + icon + native archive | `onCompleted`, `STATUS_OK` | A custom BKS whose workspace filename has no `.bks` suffix is auto-detected; V2-only signing, permissions, icon replacement, terminal native inputs, and exact Runtime Kit ABI selection are verified |
