...
包装は同じ Android 端末上のプラグインプロセス内で完結し, プロジェクトのソースは送信されません. AutoJs6 は信頼性と互換性を判定し, 返された APK を独立して検証します. 旧 `supportsRemoteBuild` 機能は, それを参照するホストのために常に宣言され, ビルド時のスイッチではなくなりました.
