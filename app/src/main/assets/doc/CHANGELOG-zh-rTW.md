...
* `優化` 受信發布工作流程新增候選隔離模式, 從綁定的宿主 Actions 構件產生五個正式簽署 APK 與 evidence, 但不建立 Release 或寫入權威相容矩陣
* `優化` 統一 Gradle 與 Python 的 Runtime Kit 驗證規則, 涵蓋摘要, 大小, 必要檔案, APK 項目及五變體一致性
* `優化` 發佈流程隨五個 APK 發佈機器可讀的 JSON 證據清單, 綁定資產摘要, 簽署憑證, 外掛/宿主版本, 相容區間, Runtime Kit ID 與協定版本
