    ...
    private ResConfigFlags readConfigFlags() throws IOException {
        ...
        if (exceedingSize > 0) {
            ...
            if (exceedingBI.equals(BigInteger.ZERO)) {
                LOGGER.fine(String.format(Locale.getDefault(), "Config flags size > %d, but exceeding bytes are all zero, so it should be ok.", KNOWN_CONFIG_BYTES));
            } else {
                LOGGER.warning(String.format(Locale.getDefault(), "Config flags size > %d. Exceeding bytes: 0x%X.", KNOWN_CONFIG_BYTES, exceedingBI));
                isInvalid = true;
            }
        }
